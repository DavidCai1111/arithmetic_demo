...
((len) ->
  ...
  console.log priorityQueue.getQue())(5)
...
((eleToFind) ->
  ...
  if result.ele != null
    ...
    console.log "使用无序表查找值为#{eleToFind}的数，此数不存在于表中，进行了#{result.count - 1}次查找")(5000)
...
((eleToFind) ->
  ...
  if result.ele != null
    ...
    console.log "使用哈希表查找值为#{eleToFind}的数，此数不存在于表中，进行了#{result.count}次查找")(5000)
